      ...
      >
        ...
        >
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            <img
              src={open ? logoGray : logoUrl || '/src/assets/logos/reactbits-gh-white.svg'}
              alt="Logo"
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none my-3" key={it.label + idx}>
                    <span
                      className={cn(
                        "sm-panel-item relative font-semibold text-[3.25rem]! cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]",
                        screen.toLowerCase() === it.label.toLowerCase() ? 'text-(--third)! pointer-events-none' : 'text-black'
                    )}
                      aria-label={it?.ariaLabel}
                      data-index={idx + 1}
                      onClick={() => {
                        toggleScreen(it.link)
                        toggleMenu()
                      }}
                    >
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div className="sm-socials mt-auto pt-8 flex flex-col gap-3" aria-label="Social links">
                <h3 className="sm-socials-title m-0 text-base font-medium text-(--third)!">Socials</h3>
                <ul
...
@media (max-width: 1024px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo-img 
@media (max-width: 640px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo-img
      `}</style>
    </div>
  );
};
